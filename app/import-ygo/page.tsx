...
export default function ImportYgoPage() {
  ...
  useEffect(() => {
    fetch('/api/decks')
      .then((res) => res.json())
      .then((data) => setDecks((data.decks ?? []).filter((deck: DeckOption) => deck.game === 'yugioh')));
  }, []);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const body: Record<string, unknown> = { text, destination, game: 'yugioh' };
  ...
  return (
    <main style={{ maxWidth: 720, margin: '0 auto', padding: '2rem 1rem' }}>
      ...
      >
        <h1 style={{ fontSize: '1.5rem', fontWeight: 600 }}>Bulk import (Yu-Gi-Oh!)</h1>
      ...
      <p style={{ color: '#666', fontSize: '0.9rem', marginBottom: '1rem' }}>
        ...
        "Main Deck:" / "Extra Deck:" / "Side Deck:" section labels are skipped automatically.
      </p>
        ...
        <fieldset style={{ border: 'none', padding: 0, marginTop: '1rem' }}>
          ...
          <label style={labelStyle}>
            <input
              ...
              style={{ marginRight: '0.5rem' }}
            />
            Collection + a deck (e.g. a structure deck you just bought)
          </label>
        </fieldset>
          ...
          {result.failed.length > 0 && (
            <div style={{ marginTop: '0.75rem' }}>
              <p style={{ color: '#b3261e', fontSize: '0.9rem' }}>
                ...
                reach YGOPRODeck (likely a brief rate limit) — try pasting just these again:
              </p>
