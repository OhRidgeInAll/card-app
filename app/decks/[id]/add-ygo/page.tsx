...
export default function AddYgoCardToDeckPage() {
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      setLoading(true);
      try {
        const res = await fetch(`/api/ygoprodeck/search?q=${encodeURIComponent(query)}`);
  ...
  return (
    <main style={{ maxWidth: 720, margin: '0 auto', padding: '2rem 1rem' }}>
      ...
      >
        <h1 style={{ fontSize: '1.5rem', fontWeight: 600 }}>Add cards to deck (Yu-Gi-Oh!)</h1>
        <button
        ...
        {results.map((card) => (
          ...
          >
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: '0.8rem', color: '#666' }}>{(card.attributes as { type?: string }).type ?? ''}</div>
            </div>
            <button
